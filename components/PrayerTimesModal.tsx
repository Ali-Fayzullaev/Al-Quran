...
export function PrayerTimesModal() {
  ...
  const [prayerSystem, setPrayerSystem] = useState<PrayerTimesSystem | null>(
    null
  );
  ...
  const loadPrayerTimes = useCallback(
    async (coordinates?: { lat: number; lng: number }) => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Ошибка загрузки времени намаза"
        );
      } finally {
        setLoading(false);
      }
            ...
            {loading ? (
              ...
              <>
                ...
                >
                  ...
                  >
                    <MapPin size={14} />
                    {prayerSystem.location.city},{" "}
                    {prayerSystem.location.country}
                  </div>
                </div>
                ...
                <div>
                  ...
                  {prayerTimesArray.map((prayer) => {
                    ...
                    const isCurrent =
                      nextPrayer?.name === prayer.name && !isPassed;
